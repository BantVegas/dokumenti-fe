...
export default function PracovnaZmluvaPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:8080/api/generate/pracovna-zmluva", formData, {
        responseType: "blob",
      });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "pracovna-zmluva.pdf");
  ...
  const handlePrintTemplate = () => {
    window.open("/templates/pracovna-zmluva-template.pdf", "_blank");
  };

  return (
    <main
      className="min-h-screen flex flex-col justify-between bg-cover bg-center"
      style={{ backgroundImage: "url('/images/pracovna-zmluva-hero.png')" }}
    >
      <div className="flex-grow flex items-center justify-center p-6">
        ...
        >
          <h2 className="text-xl font-bold mb-6">Pracovná zmluva</h2>
          <form className="space-y-4" onSubmit={handleSubmit}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <input name="miestoPrace" placeholder="Miesto výkonu práce" value={formData.miestoPrace} onChange={handleChange} className="px-4 py-2 border rounded-lg" required />
              <input name="pracovnyCas" placeholder="Pracovný čas (napr. 40h/týždeň)" value={formData.pracovnyCas} onChange={handleChange} className="px-4 py-2 border rounded-lg" required />
              <input name="datumNastupu" placeholder="Dátum nástupu do práce" value={formData.datumNastupu} onChange={handleChange} className="px-4 py-2 border rounded-lg" required />
              <input name="pozicia" placeholder="Pozícia" value={formData.pozicia} onChange={handleChange} className="px-4 py-2 border rounded-lg" required />
              <input name="mzda" placeholder="Mzda (EUR/mesiac)" value={formData.mzda} onChange={handleChange} className="px-4 py-2 border rounded-lg" required />
            </div>
